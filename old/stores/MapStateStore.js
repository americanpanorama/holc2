...
const MapStateStore = {
  ...
  setViewFromBounds: function (bounds) {
    ...
    // this.data.center = lBounds.getCenter();
    const centerZoom = this.data.theMap._getBoundsCenterZoom(bounds, {paddingBottomRight: [DimensionsStore.getDimensions().sidebarStyle.width, 0]});
    this.data.zoom = centerZoom.zoom;
    this.data.center = centerZoom.center;
    this.emit(AppActionTypes.storeChanged);
    this.viewInitialized();
  },

  setViewFromBoundsOffset: function (bounds) {
    const lBounds = L.latLngBounds(bounds);
    if (offset) {
      const extendedBounds = lBounds.pad(0.33);
      const extendedEast = extendedBounds.getEast();
      lBounds.extend([lBounds.getNorth(), extendedBounds.getEast()]);
      console.log(bounds, lBounds, extendedEast);
    }
    ...
    const waitingId = setInterval(() => {
      if (action.value == CityStore.getId()) {
        ...
        if (action.selectedByUser && MapStateStore.getTheMap() !== null) {
          let mapIds = CitiesStore.getMapIds(CityStore.getId()),
            bounds = (CityStore.getPolygonsBounds()) ? CityStore.getPolygonsBounds() : RasterStore.calculateMapsBounds(mapIds),
            outerRingRadius = CityStore.getOuterRingRadius(),
            loopCenter = CityStore.getLoopLatLng();

          // calculate an adjusted bounding box to position city ring center 1/3 of the map width from the left to account for the sidebar
          // if (outerRingRadius) {
          //  const dimensions = DimensionsStore.getDimensions();
          //  const ringBounds = L.latLng(loopCenter).toBounds(outerRingRadius);
          //  if (dimensions.windowWidth * 2 / 3 > dimensions.mapStyle.height) {
          //    // calculate the ratio of the area falling the the left or right of the ring bounds
          //    const eastWestPaddingRatio = dimensions.windowWidth * 2 / 3 / dimensions.mapStyle.height / 2;
          //    // calculate the lng distance between the east and west bounds of the ring and the center
          //    const lngRadius = loopCenter[1] - ringBounds.getWest();
          //    const adjustedWestLng = ringBounds.getWest() - lngRadius * eastWestPaddingRatio;
          //    console.log(dimensions.windowWidth * 2 / 3, dimensions.mapStyle.height, eastWestPaddingRatio, lngRadius, ringBounds.getWest(), adjustedWestLng);
          //  }
          //  const extendedBounds = ringBounds.pad(0.75);
          //  const extendedEast = extendedBounds.getEast();
          //  ringBounds.extend([ringBounds.getNorth(), extendedBounds.getEast()]);
          //  bounds = ringBounds;
          //  console.log(bounds, extendedBounds);
          // }
          MapStateStore.setViewFromBounds(bounds);
        }
      }
